...
public class SysDeptController extends BaseController {
    ...
    public String edit(@PathVariable("deptId") Long deptId, ModelMap mmap) {
        ...
        mmap.put("dept", dept);
        List<WeekVO> weekList = DateUtils.getWeekList();
        if (StringUtils.isNotNull(dept.getWeeks())) {
            String[] houseFeatureIds = dept.getWeeks().split(",");
            for (WeekVO dict : weekList) {
                for (int i = 0; i < houseFeatureIds.length; i++) {
                    if (dict.getNum().equals(Convert.toInt(houseFeatureIds[i]))) {
                        dict.setFlag(true);
                        break;
                    }
                }
     ...
     * 加载部门列表树
     */
    @GetMapping("/treeDataByCompany")
    @ResponseBody
    public List<Ztree> treeDataByCompany() {
/*        SysDept sysDept = new SysDept();
        ...
        List<Ztree> ztrees = deptService.treeDataByCompany(sysDept);
        return ztrees;*/
        SysDept dept = new SysDept();
        dept.setDeptId(ShiroUtils.getSysUser().getDeptId());
     ...
     * 考勤设置
     */
    @GetMapping("/setClocking")
    public String setClocking(Long deptId, ModelMap mmap) {
        SysDept dept = deptService.selectDeptById(deptId);
        if (StringUtils.isNotNull(dept) && 100L == deptId) {
            dept.setParentName("无");
        }
     ...
     * @return
     */
    @PostMapping("/selectDeptListById")
    @ResponseBody
    public List<SysDept> selectDeptListById(Long companyId) {
        SysDept dept = new SysDept();
