...
import com.wlwq.params.examine.TargetContentParamVo;
 ...
 */
@RestController
@RequestMapping(value = "/api/targetTrainingRecordFlow")
@AllArgsConstructor
public class TargetTrainingRecordFlowApiController extends ApiController {
    ...
    public ApiResult targetTrainingRecordFlowSub(HttpServletRequest request, @RequestBody @Validated TargetTrainingRecordParamVO params, BindingResult bindingResult) {
        ...
        TargetTrainingRecord meetingTraining = targetTrainingRecordService.selectTargetTrainingRecordById(params.getTargetTrainingRecordId());
        if (meetingTraining == null) {
            return ApiResult.fail("目标训练记录不存在！");
        }
        ...
        examineInitiate.setExamineModuleId(28);
    ...
    @RepeatSubmit
    @Transactional(rollbackFor = Exception.class)
    @PostMapping("/goalCompletionFlowSub")
    public ApiResult goalCompletionFlowSub(HttpServletRequest request, @RequestBody @Validated TargetContentParamVo params, BindingResult bindingResult) {
        if (bindingResult.hasErrors()) {
            return ApiResult.fail(Objects.requireNonNull(bindingResult.getFieldError()).getDefaultMessage());
        }
        ApiAccount account = accountService.selectApiAccountById(tokenService.getAccountId(request));
        ...
        examineInitiate.setSynopsis(account.getBriefIntroduction());
        examineInitiate.setDeptName(params.getDeptName());
        examineInitiate.setExamineModuleId(29);
        ...
        examineInitiate.setParentTitle(params.getParentTitle());
        examineInitiate.setSubTitle(params.getSubTitle());
        examineInitiate.setSubContent(params.getSubContent());
        examineInitiate.setTargetTrainingRecordContentId(params.getTargetTrainingRecordContentId());
        examineInitiate.setDocumentationUrl(params.getDocumentationUrl());
        examineInitiate.setExamineTag(IdUtil.getSnowflake(1, 1).nextIdStr());
        int count = meetingInitiateService.insertMeetingExamineInitiate(examineInitiate);
        if (count <= 0) {
            throw new ApiException("提交失败");
    ...
    public ApiResult initiateDetail(@RequestParam(defaultValue = "0") String initiateId) {
        ...
        if (examineInitiate.getExamineModuleId() == 28) {
            ...
            if (examineInitiate.getExamineModuleId() == 29) {
                // 查询文件
        ...
        TargetTrainingRecord targetTrainingRecord = targetTrainingRecordService.selectTargetTrainingRecordById(examineInitiate.getMeetingTrainingId());
    ...
    public ApiResult examineDetail(HttpServletRequest request, @RequestParam(defaultValue = "0") String flowAccountId,
        ...
        TargetTrainingRecord targetTrainingRecord = targetTrainingRecordService.selectTargetTrainingRecordById(examineInitiate.get("meetingTrainingId").toString());
        if (targetTrainingRecord == null) {
            return ApiResult.fail("没有此记录数据！");
        }
        List<TargetTrainingRecordContent> targetContentList = targetTrainingRecordContentService.selectTargetTrainingRecordContentList(TargetTrainingRecordContent
        ...
        examineInitiate.put("targetContentList", targetContentList);
        examineInitiate.put("targetTrainingRecord", targetTrainingRecord);

        // 审批类型ID
        if (Convert.toInt(examineInitiate.get("examineModuleId")) == 28) {
            ...
            if (Convert.toInt(examineInitiate.get("examineModuleId")) == 29) {
                // 查询文件
    ...
    public ApiResult editLeaveSub(HttpServletRequest request, @RequestBody @Validated TargetTrainingRecordParamVO params,
        ...
        if (status != 1 && status != 4 && status != 5 && status != 3) {
            return fail("只有在待审批或者已驳回或者已撤回或者已通过的状态下才能修改！");
        }
        // 查询用户所在的部门的某一各类型是否添加审批流程
        MeetingExamineFlow examineFlow = meetingFlowService.selectMeetingExamineFlow(MeetingExamineFlow.builder()
                .companyId(account.getCompanyId())
        ...
        // 异步操作
        fileSave(params.getFileList(), initiate.getExamineInitiateId());
